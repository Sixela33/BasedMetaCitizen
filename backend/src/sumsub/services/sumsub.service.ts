...
export class SumsubService {
    ...
    async onModuleInit() {
        const _sumsubSecretKey = process.env.SUMSUB_SECRET_KEY;
        const _sumsubPublicKey = process.env.SUMSUB_PUBLIC_KEY;

        if (!_sumsubSecretKey || !_sumsubPublicKey) {
            throw new Error('SUMSUB_SECRET_KEY and SUMSUB_PUBLIC_KEY must be set');
        }

        this.sumsubSecretKey = _sumsubSecretKey;

        this.sumsubClient = axios.create({
            baseURL: 'https://api.sumsub.com',
            headers: {
                'Content-Type': 'application/json',
                'X-App-Token': _sumsubPublicKey
            }
        });
